...
(*
   ...
   3. Determinization (compile)
      ...
      Why two kinds of cells. A tag is a name and a register is storage:
      one tag needs several registers when live paths disagree about its
      value. The code reading the captures, on the other hand, is
      generated once per rule and reads fixed cells, while the winning
      path's registers depend on the accepting state that was reached; the
      canonical cells are that fixed meeting point. They are never used
...
(* Where the value of a logical tag lives, along one NFA path, while a
   transition is being computed: in a memory cell ([Cell]), or in a write
   that the pending transition will perform ([Pending]). Within one
   transition, two [Pending] addresses of the same tag denote the same
   register exactly when they hold the same write. *)
type addr = Cell of cell | Pending of write

(* One active NFA path: the node it reached and, for each logical tag
   written along the path, where the recorded value lives. A DFA state is a
   list of configurations in priority order. It exists in two forms:
   - a [candidate] comes out of [eps_closure]: its tags live in cells or in
     the writes of the transition being computed;
   - a [stored] state is what the state table keeps: every tag lives in a
     memory cell. *)
...
(* [eps_closure seeds] computes the priority-ordered epsilon closure of
   ...
   Tag writes encountered along the way become [Pending] addresses. *)
let eps_closure (seeds : candidate list) : candidate list =
...
(* [split_moves moves] partitions the character transitions leaving a DFA
   ...
   Invariant: the pieces built so far are pairwise disjoint, and a move's
   configuration is appended last to every piece it overlaps; moves come
   in priority order, so seeds stay in priority order. *)
let split_moves (moves : (Cset.t * candidate) list) :
    (Cset.t * candidate list) list =
...
(* === Determinization state ===
   ...
   The subset construction threads mutable state, made explicit:
   [Registers] (memory-cell allocation) and [state_table] (the DFA states
   discovered so far), grouped with the compiled rules in [ctx]. *)

(* Memory-cell allocation. Canonical cells [0 .. num_logical - 1] (cell =
   logical tag id) are written only by final operations and read by the
   generated binding-extraction code; working registers live above them. *)
...
module State_key : sig
  ...
  val of_candidate : candidate list -> t
...
type state_table = {
  ...
  configs : (int, stored list) Hashtbl.t;
...
(* [add_state tbl key configs] registers a new state and returns its
   number. *)
let add_state (tbl : state_table) (key : State_key.t) (configs : stored list) :
    int =
...
(* [register_moves candidate existing] is what a transition reaching an
   existing state must perform: the register moves that realign the
   candidate's registers with the stored state's. Equal keys guarantee
   each destination cell gets a single consistent move. The result is a
   parallel move (Copy sources observe the pre-transition state), listed
   by destination.
   (ocamllex: [move_to]) *)
let register_moves (candidate : candidate list) (existing : stored list) :
    tag_op list =
...
(* [final_ops_of regs accepting] materializes the registers of the
   accepting configuration into the canonical cells (cell = logical tag
   id), to run just before [Sedlexing.mark]. Sources are working registers
   and destinations canonical cells, so the copies never interfere with
   each other. *)
let final_ops_of (regs : Registers.t) (accepting : stored) : tag_op list =
  TagMap.fold
    (fun tag cell acc ->
...
(* [accept_of ctx configs] resolves rule priority: an accepting state
   carries the lowest-numbered rule whose final node is in [configs], i.e.
   the highest-priority one under the first-match semantics of
   [match%sedlex], and the operations materializing the registers of that
   node's configuration. *)
let accept_of (ctx : ctx) (configs : stored list) : accept option =
...
(* [find_or_add_state ctx candidate] returns the state number for
...
(* [build_state ctx num configs] computes and stores the definition of
   state [num]. *)
and build_state (ctx : ctx) (num : int) (configs : stored list) : unit =
  let trans = transitions ctx configs in
  let accept = accept_of ctx configs in
  Hashtbl.add ctx.tbl.defs num { trans; accept }

(* [transitions ctx configs] builds the outgoing transitions of one DFA
...
and transitions (ctx : ctx) (configs : stored list) :
