...
let new_tagged_node tag =
  incr cur_id;
  { id = !cur_id; eps = []; trans = []; tag = Some tag }
...
let rec add_node (state, tags) node =
  if List.memq node state then (state, tags)
    ...
    let tags = match node.tag with Some t -> t :: tags | None -> tags in
...
type dfa_state = {
  trans : (Cset.t * int * int list) array;
  finals : bool array;
}
        ...
        (fun (cset, target, tags) ->
          ...
          let label =
            if tags = [] then label
              ...
              label ^ " {t"
              ^ String.concat ",t" (List.map string_of_int tags)
              ^ "}"
          in
