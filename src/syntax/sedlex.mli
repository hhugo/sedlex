...
(** [optimize ~live compiled] eliminates dead tags and remaps live ones to a
    ...
    generated code. Tags not in [live] are stripped from all DFA transitions
    and [init_tags], and [num_tags] is reduced accordingly. Returns the
    optimized compiled result and a mapping array [old_tag → new_tag]
    ([-1] for dead tags, identity when nothing changed). *)
